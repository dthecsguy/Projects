...
public:
    ...
    vec3 Emitted_Light(const vec3& vector_to_light) const
    {
        ...
        double cosphi = dot(direction, double(-1) * vector_to_light.normalized());

        return (color * brightness * pow(cosphi, falloff_exponent)) / (pi * 4 * vector_to_light.magnitude_squared());
    }
};
#endif
